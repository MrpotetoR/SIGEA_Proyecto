<x-panel title="Mi Horario" panelNombre="Panel Alumno">
    ...
    <div class="space-y-5">
        ...
        @php
            // Paleta por dia (misma identidad visual que el horario del docente)
            $colores = [
            ...
            // Rango dinamico segun el horario real (fallback 07-15)
            $minH = 24; $maxH = 0;
            foreach ($dias as $d) {
            ...
            // Mapa de ocupacion: [dia][HH:00] = clase (inicio) | 'SPAN' (cubierta)
            $ocupado = [];
            foreach ($dias as $d) {
                foreach ($horario[$d] ?? [] as $c) {
                    ...
                    @foreach($horas as $hora)
                        <tr class="h-16">
                            ...
                            @foreach($dias as $dia)
                                ...
                                @if($cell === 'SPAN')
                                    {{-- Celda cubierta por rowspan previo --}}
                                @elseif($cell)
                                    @php
                                    ...
                                    <td rowspan="{{ $rowspan }}" class="p-1.5 align-middle border-l border-gray-100 dark:border-gray-700">
                                        <div class="h-full flex flex-col justify-center p-2.5 rounded-xl border {{ $c['bg'] }} {{ $c['border'] }} text-center">
                                            ...
                                            <p class="text-[10px] mt-1 {{ $c['sub'] }} truncate">{{ $cell->docente?->nombre_completo ?? 'Sin docente' }}</p>
                                            <p class="text-[10px] mt-0.5 {{ $c['sub'] }} opacity-80">{{ \Carbon\Carbon::parse($cell->hora_inicio)->format('H:i') }} – {{ \Carbon\Carbon::parse($cell->hora_fin)->format('H:i') }}</p>
                                        </div>
                                    </td>
                                @else
