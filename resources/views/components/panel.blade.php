...
<body class="antialiased bg-[#f5f5f0]">
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="w-[220px] bg-white border-r border-gray-200/60 flex flex-col flex-shrink-0">
            ...
            {{-- Navegación --}}
            <nav class="flex-1 overflow-y-auto custom-scrollbar px-3 space-y-0.5 pb-4 text-gray-600 ">
                {{ $nav ?? '' }}
            </nav>
            ...
            <div class="p-4 flex items-center gap-3">
                ...
                <div
                    class="w-9 h-9 rounded-full bg-gradient-to-br from-black to-gray-800 flex items-center justify-center text-white text-xs font-bold shadow-sm">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <header class="bg-[#f5f5f0] flex items-center justify-between px-7 h-14 flex-shrink-0">
                ...
                <div class="flex items-center gap-1">
                    ...
                    <button id="btn-chat-toggle" onclick="toggleChatbot()"
                        class="p-2.5 rounded-xl text-gray-400 hover:text-gray-700 hover:bg-white/60 transition-colors relative">
                        <svg class="w-[18px] h-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                            stroke-width="1.8">
                            <path stroke-linecap="round" stroke-linejoin="round"
            ...
            @if (session('success'))
                <div
                    class="mx-7 mb-3 bg-emerald-50 border border-emerald-200 text-emerald-700 px-4 py-3 rounded-2xl text-[13px] flex items-center gap-2 fade-in">
            ...
            @if (session('error'))
                <div
                    class="mx-7 mb-3 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-2xl text-[13px] flex items-center gap-2 fade-in">
                    <svg class="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                        stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round"
    ...
    @auth
        ...
        @if($chatRoute)
            <div id="chatbot-panel"
                class="fixed z-50 right-4 bg-white rounded-2xl border border-gray-100 overflow-hidden flex-col" style="top: 60px; width: 300px; display: none;
                ...
                <div id="chat-messages" class="overflow-y-auto p-5 space-y-3 flex-1"
                    ...
                    <div class="text-center py-4" id="chat-welcome">
                        <div class="w-14 h-14 mx-auto rounded-2xl bg-gradient-to-br from-black to-gray-800
                        ...
                        <h3 class="text-[16px] font-bold text-gray-800 mb-1">¿En que te ayudo?</h3>
                    ...
                    <div class="flex flex-wrap gap-1.5 justify-center" id="sugerencias">
                        @foreach($chatSugerencias as $sug)
                            <button onclick="enviarSugerencia(this)" class="text-[11px] font-medium bg-gray-100 hover:bg-gray-200 text-gray-600
                ...
                <div class="p-4 border-t border-gray-100 flex-shrink-0">
                    <div class="flex items-center gap-2 bg-gray-50 rounded-xl px-3.5 py-2.5">
                        ...
                        <button onclick="enviarMensaje()" class="w-8 h-8 bg-gray-900 rounded-lg flex items-center justify-center
                                           hover:bg-gray-700 flex-shrink-0 transition-colors">
            ...
            <script>
                ...
                function toggleChatbot() {
                    ...
                    if (!chatOpen) {
                        ...
                        btn.classList.add('text-gray-700', 'bg-white/60');
                    } else {
                        panel.classList.remove('chat-open');
                        panel.classList.add('chat-closed');
                        setTimeout(() => { panel.style.display = 'none'; }, 180);
                        btn.classList.remove('text-gray-700', 'bg-white/60');
                    }
                    chatOpen = !chatOpen;
                }
