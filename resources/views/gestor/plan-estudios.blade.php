<x-panel title="Plan de Estudios" panelNombre="Panel Gestor Escolar">
    <x-slot name="nav">
        @include('partials.gestor-nav')
    </x-slot>

    <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">Malla curricular de <span
            class="font-semibold text-gray-700 dark:text-gray-300">{{ $carrera?->nombre_carrera ?? 'Sin carrera' }}</span>
    </p>

    @if($materias->isEmpty())
        <div
            class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm dark:shadow-gray-900/20 border border-gray-100 dark:border-gray-700 p-12 text-center">
    ...
    @else
        <div class="space-y-6">
            @foreach($materias as $cuatrimestre => $materiasCuat)
                <div
                    class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm dark:shadow-gray-900/20 border border-gray-100 dark:border-gray-700 overflow-hidden">
                    ...
                    <div
                        class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0 divide-x divide-y divide-gray-100 dark:divide-gray-700">
