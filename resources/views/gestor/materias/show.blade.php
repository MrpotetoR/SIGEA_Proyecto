<x-panel title="Detalle de Materia" panelNombre="Panel Gestor Escolar">
    <x-slot name="nav">@include('partials.gestor-nav')</x-slot>

    <div class="max-w-2xl space-y-6">
        <a href="{{ route('gestor.materias.index') }}" class="text-sm text-[#0606F0] dark:text-blue-400 hover:underline inline-block">← Volver</a>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow dark:shadow-gray-900/20 p-6 border border-transparent dark:border-gray-700">
            <div class="flex items-start justify-between mb-6">
                ...
                <a href="{{ route('gestor.materias.edit', $materia) }}"
            ...
            <dl class="grid grid-cols-2 gap-x-8 gap-y-4 text-sm">
                ...
                <div>
                    ...
                    <dd class="mt-1 font-medium text-gray-800 dark:text-gray-200">
                        {{ $materia->horas_semana ? $materia->horas_semana . ' hrs' : 'No definidas' }}
                    </dd>
                </div>
                <div>
            ...
            <div class="flex gap-3 pt-6 mt-6 border-t dark:border-gray-700">
                ...
                <form method="POST" action="{{ route('gestor.materias.destroy', $materia) }}"
                      data-udea-confirm
                      data-confirm-title="Eliminar materia"
                      data-confirm-message="¿Eliminar la materia <strong>&quot;{{ $materia->nombre_materia ?? $materia->nombre ?? 'seleccionada' }}&quot;</strong>?"
