<x-panel title="Nuevo Docente" panelNombre="Panel Gestor Escolar">
    <x-slot name="nav">@include('partials.gestor-nav')</x-slot>

    <div class="max-w-5xl">
        <a href="{{ route('gestor.docentes.index') }}" class="text-sm text-[#0606F0] dark:text-blue-400 hover:underline mb-4 inline-block">← Volver</a>

        <form method="POST" action="{{ route('gestor.docentes.store') }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div id="carreras-container" class="space-y-2">
                                <div class="carrera-row flex gap-2">
                                    <select name="carreras[]"
                                            class="flex-1 border dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-400 focus:outline-none">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow dark:shadow-gray-900/20 p-6 border border-transparent dark:border-gray-700 flex flex-col" style="max-height: calc(100vh - 180px);">
                    ...
                    <div class="overflow-y-auto flex-1 custom-scrollbar pr-1 space-y-3">
                        @php $documentosOpcionales = ['cedula_profesional']; @endphp
                        @foreach(\App\Models\DocumentoDocente::TIPOS as $key => $label)
                            @php $esOpcional = in_array($key, $documentosOpcionales, true); @endphp
                            <div class="border dark:border-gray-600 rounded-lg p-3 bg-gray-50 dark:bg-gray-700/50 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                                ...
                                <input type="file" name="documentos[{{ $key }}]" accept="application/pdf" {{ $esOpcional ? '' : 'required' }}
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow dark:shadow-gray-900/20 p-4 border border-transparent dark:border-gray-700 flex gap-3">
                ...
                <a href="{{ route('gestor.docentes.index') }}"
...
function toggleCarreras() {
    // Especialidad ahora es opcional — no se bloquean carreras.
}
