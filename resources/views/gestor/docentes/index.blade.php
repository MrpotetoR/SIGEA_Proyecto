<x-panel title="Docentes" panelNombre="Panel Gestor Escolar">
    ...
    <form method="GET"
        ...
        <a href="{{ route('gestor.docentes.create') }}"
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow dark:shadow-gray-900/20 border border-transparent dark:border-gray-700 flex flex-col min-h-0"
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    @forelse($docentes as $d)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            <td class="px-4 py-3 font-medium text-gray-900 dark:text-gray-100">{{ $d->nombre_completo }}
                            </td>
                            ...
                            <td class="px-4 py-3 text-center text-gray-600 dark:text-gray-400">
                                @if(is_null($d->horas_contrato))
                                    <span
                                        class="px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300">Planta</span>
                                @else
                            ...
                            <td class="px-4 py-3 text-center text-gray-600 dark:text-gray-400">
                                @if($d->es_tutor)
                                    <span
                                        class="px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300">Sí</span>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-center">
                                <div class="flex justify-center gap-2">
                                    <a href="{{ route('gestor.docentes.show', $d) }}"
                                        class="text-[#0606F0] dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300 font-medium">Ver</a>
                                    <a href="{{ route('gestor.docentes.edit', $d) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-10 text-center text-gray-400 dark:text-gray-400">No hay docentes
                                registrados.</td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if($docentes instanceof \Illuminate\Pagination\LengthAwarePaginator)
            <div class="px-4 py-3 border-t dark:border-gray-700 flex-shrink-0">{{ $docentes->links() }}</div>
        @endif
    </div>
</x-panel>
