<x-panel title="Editar Docente" panelNombre="Panel Gestor Escolar">
    <x-slot name="nav">@include('partials.gestor-nav')</x-slot>

    <div class="max-w-5xl">
        <a href="{{ route('gestor.docentes.show', $docente) }}" class="text-sm text-[#0606F0] dark:text-blue-400 hover:underline mb-4 inline-block">← Volver</a>

        @php
            $docsExistentes = $docente->documentos->keyBy('tipo');
        @endphp

        <form method="POST" action="{{ route('gestor.docentes.update', $docente) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf @method('PUT')
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow dark:shadow-gray-900/20 p-6 border border-transparent dark:border-gray-700 flex flex-col" style="max-height: calc(100vh - 180px);">
                    ...
                    <div class="overflow-y-auto flex-1 custom-scrollbar pr-1 space-y-3">
                        ...
                        @foreach(\App\Models\DocumentoDocente::TIPOS as $key => $label)
                            @php
                                $doc = $docsExistentes->get($key);
                                $esOpcional = in_array($key, $documentosOpcionales, true);
                            @endphp
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow dark:shadow-gray-900/20 p-4 border border-transparent dark:border-gray-700 flex gap-3">
                ...
                <a href="{{ route('gestor.docentes.show', $docente) }}"
