<x-panel title="Overview" panelNombre="Panel Gestor Escolar">
...
<div class="space-y-5">
    ...
    @unless($stats['ciclo_activo'])
        <div class="flex items-center gap-3 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-700 rounded-2xl px-5 py-3.5 text-[13px]">
            ...
            <span class="text-amber-700 dark:text-amber-300">No hay un ciclo escolar activo.</span>
            <a href="{{ route('gestor.ciclos.create') }}" class="ml-auto text-amber-700 dark:text-amber-300 font-semibold hover:underline">Crear ciclo →</a>
        </div>
    @endunless
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm dark:shadow-gray-900/20 overflow-hidden">
        <div class="grid grid-cols-2 md:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-gray-100 dark:divide-gray-700">
            ...
            <div class="p-5 card-hover">
                ...
                <a href="{{ route('gestor.alumnos.index') }}" class="text-[11px] text-[#0606F0] dark:text-blue-400 hover:underline mt-1.5 block">Ver listado →</a>
            </div>

            {{-- Bajas temporales --}}
            <div class="p-5 card-hover">
                <div class="flex items-center gap-2 mb-3">
                    <span class="w-6 h-6 rounded-lg bg-amber-100 dark:bg-amber-900/40 flex items-center justify-center">
                    ...
                    <span class="text-[13px] font-semibold text-gray-700 dark:text-gray-300">Bajas Temporales</span>
                </div>
                <p class="text-[28px] font-bold text-gray-900 dark:text-gray-100 leading-none">{{ $stats['bajas_temporales'] }}</p>
                <a href="{{ route('gestor.alumnos.index', ['estatus' => 'baja_temporal']) }}" class="text-[11px] text-amber-500 dark:text-amber-400 hover:underline mt-1.5 block">Ver bajas →</a>
            </div>
            ...
            <div class="p-5 card-hover">
                ...
                <a href="{{ route('gestor.docentes.index') }}" class="text-[11px] text-sky-500 dark:text-sky-400 hover:underline mt-1.5 block">Ver listado →</a>
            </div>

            {{-- Carreras o Grupos segun contexto --}}
            <div class="p-5 card-hover">
                <div class="flex items-center gap-2 mb-3">
                    <span class="w-6 h-6 rounded-lg bg-sky-100 dark:bg-sky-900/40 flex items-center justify-center">
                        <svg class="w-3.5 h-3.5 text-sky-600 dark:text-sky-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    <span class="text-[13px] font-semibold text-gray-700 dark:text-gray-300">{{ $stats['cuarta_kpi_label'] ?? 'Carreras' }}</span>
                </div>
                <p class="text-[28px] font-bold text-gray-900 dark:text-gray-100 leading-none">{{ $stats['cuarta_kpi_valor'] ?? $stats['total_carreras'] }}</p>
                @if(($stats['es_bachi'] ?? false))
                    <a href="{{ route('gestor.grupos.index') }}" class="text-[11px] text-sky-500 dark:text-sky-400 hover:underline mt-1.5 block">Ver grupos →</a>
                @else
                    <a href="{{ route('gestor.carreras.index') }}" class="text-[11px] text-sky-500 dark:text-sky-400 hover:underline mt-1.5 block">Ver carreras →</a>
                @endif
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm dark:shadow-gray-900/20 card-hover">
            <div class="flex items-center justify-between px-5 pt-5 pb-3">
                ...
                <a href="{{ route('gestor.ciclos.index') }}"
            ...
            <div class="px-5 pb-5">
                ...
                @else
                    <div class="text-center py-8">
                        ...
                        <a href="{{ route('gestor.ciclos.create') }}"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm dark:shadow-gray-900/20 card-hover">
            ...
            <div class="px-5 pb-5 grid grid-cols-2 gap-2">
                @php
                $accesos = [
                    ['route' => 'gestor.alumnos.create',  'label' => 'Nuevo Alumno',
                    ...
                    ['route' => 'gestor.docentes.create', 'label' => 'Nuevo Docente',
                    ...
                    ['route' => 'gestor.inscripciones',   'label' => 'Inscripciones',
