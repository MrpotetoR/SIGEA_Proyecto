<x-panel title="Editar Ciclo Escolar" panelNombre="Panel Gestor Escolar">
    <x-slot name="nav">@include('partials.gestor-nav')</x-slot>
    <div class="max-w-lg">
        <a href="{{ route('gestor.ciclos.index') }}" class="text-sm text-[#0606F0] dark:text-blue-400 hover:underline mb-6 inline-block">← Volver</a>
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow dark:shadow-gray-900/20 p-6 border border-transparent dark:border-gray-700">
            <div class="mb-5 text-xs text-gray-500 dark:text-gray-400 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-lg px-3 py-2">
                ℹ Duración fija: <strong>3 años y 4 meses</strong>. El nombre y la fecha de fin se recalculan al cambiar la fecha de inicio.
            </div>
            <form method="POST" action="{{ route('gestor.ciclos.update', $ciclo) }}" class="space-y-5">
                @csrf @method('PUT')
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Fecha de inicio *</label>
                    <input type="date" name="fecha_inicio" id="fecha_inicio" value="{{ old('fecha_inicio', $ciclo->fecha_inicio->toDateString()) }}" required
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <div id="prev_nombre" class="w-full border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50 rounded-lg px-3 py-2 text-sm text-gray-600 dark:text-gray-300">{{ $ciclo->nombre }}</div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Fecha de fin</label>
                        <div id="prev_fin" class="w-full border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50 rounded-lg px-3 py-2 text-sm text-gray-600 dark:text-gray-300">{{ $ciclo->fecha_fin->toDateString() }}</div>
                    </div>
                </div>
                <div class="flex gap-3 pt-4 border-t dark:border-gray-700">
                    <button type="submit" id="btn_submit" class="bg-blue-700 hover:bg-blue-800 dark:bg-[#0606F0] dark:hover:bg-blue-400 text-white px-6 py-2.5 rounded-lg text-sm font-semibold transition-colors">Guardar</button>
                    <a href="{{ route('gestor.ciclos.index') }}" class="bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 px-6 py-2.5 rounded-lg text-sm font-medium transition-colors">Cancelar</a>
                </div>
            </form>
        </div>
    </div>
...
(function() {
    ...
    function calc() {
        ...
        if (aniosUsados.includes(d.getFullYear())) {
            warn.textContent = 'Ya existe otro ciclo que inicia en ' + d.getFullYear() + '. Sólo se permite un ciclo por año de inicio.';
