<x-panel title="Grupo {{ $grupo->clave_grupo }}" panelNombre="Panel Gestor Escolar">
    <x-slot name="nav">
    ...
    <div class="max-w-3xl">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm dark:shadow-gray-900/20 border border-gray-100 dark:border-gray-700 p-6 mb-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('gestor.grupos.edit', $grupo->id_grupo) }}" class="px-4 py-1.5 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 text-xs font-medium rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-colors">Editar</a>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm dark:shadow-gray-900/20 border border-gray-100 dark:border-gray-700 p-6">
            ...
            @if($disponibles->isNotEmpty())
                <form action="{{ route('gestor.grupos.inscribir', $grupo->id_grupo) }}" method="POST" class="flex items-center gap-2 mb-4">
                    @csrf
                    <select name="id_alumno" required class="flex-1 text-sm rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        ...
                        @foreach($disponibles as $al)
                            <option value="{{ $al->id_alumno }}">{{ $al->nombre_completo }} ({{ $al->id_alumno_publico }})</option>
                        @endforeach
                    </select>
                        ...
                        <div class="flex items-center justify-between py-2 px-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-400 dark:text-gray-500">{{ $insc->alumno?->id_alumno_publico }}</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-3">
