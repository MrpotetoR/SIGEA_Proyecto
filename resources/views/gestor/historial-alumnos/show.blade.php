<x-panel title="Historial de {{ $alumno->nombre_completo }}" panelNombre="Panel Gestor Escolar">
    <x-slot name="nav">
    ...
    {{-- Boton volver --}}
    <div class="mb-5">
        <a href="{{ route('gestor.alumnos') }}" class="inline-flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 transition-colors">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm dark:shadow-gray-900/20 border border-gray-100 dark:border-gray-700 p-6 mb-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">ID: <span class="font-mono">{{ $alumno->id_alumno_publico }}</span> | Cuatrimestre: {{ $alumno->cuatrimestre_actual ?? 'N/A' }} | Estatus: <span class="capitalize">{{ $alumno->estatus }}</span></p>
            </div>
        </div>
    </div>
