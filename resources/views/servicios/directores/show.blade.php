<x-panel title="Detalle Director" panelNombre="Servicios Escolares">
    ...
    <div class="max-w-3xl space-y-6">
        <div class="flex items-center justify-between">
            <a href="{{ route('servicios.directores.index') }}" class="text-sm text-indigo-600 hover:underline">← Volver</a>
        ...
        <div class="bg-white rounded-xl shadow p-6">
            <h2 class="text-xl font-bold text-gray-900 mb-4">{{ $director->nombre_completo }}</h2>
            <dl class="grid grid-cols-2 gap-4 text-sm">
                <div>
                    <dt class="text-gray-500">Correo</dt>
                    <dd class="font-medium">{{ $director->user?->email }}</dd>
                </div>
                <div>
                    <dt class="text-gray-500">Especialidad</dt>
                    <dd class="font-medium">{{ $director->especialidad ?? '—' }}</dd>
                </div>
                <div>
                ...
                <div>
                    <dt class="text-gray-500">Carrera(s) que dirige</dt>
                    <dd>
