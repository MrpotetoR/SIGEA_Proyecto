            ...
            <form method="POST" action="{{ route('servicios.noticias.update', $noticia) }}" class="space-y-5">
                ...
                <div>
                    ...
                    <textarea name="contenido" rows="6" required
                              class="w-full border dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-400 focus:outline-none">{{ old('contenido', $noticia->contenido) }}</textarea>
                </div>
                <div>
