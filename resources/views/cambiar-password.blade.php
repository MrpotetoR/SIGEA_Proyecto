...
<div class="max-w-lg mx-auto space-y-5">
    ...
    <div class="text-center">
        ...
        <p class="text-[13px] text-gray-400 mt-1">Asegurate de usar una contrasena segura de al menos 8 caracteres.</p>
    </div>

    @if(session('success'))
        <div class="bg-emerald-50 border border-emerald-200 text-emerald-700 px-4 py-3 rounded-2xl text-[13px] flex items-center gap-2">
    ...
    <form method="POST" action="{{ route('cambiar-password.update') }}" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-5">
        @csrf
        @method('PUT')

        {{-- Contrasena actual --}}
        <div>
            <label class="text-[12px] font-semibold text-gray-500 uppercase tracking-wider mb-2 block">Contrasena actual</label>
            <div class="relative">
            ...
            @error('current_password')
                <p class="text-red-500 text-[11px] mt-1.5 flex items-center gap-1">
                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4m0 4h.01"/></svg>
                    {{ $message }}
                </p>
            @enderror
        </div>
        ...
        {{-- Nueva contrasena --}}
        <div>
            <label class="text-[12px] font-semibold text-gray-500 uppercase tracking-wider mb-2 block">Nueva contrasena</label>
        ...
        {{-- Confirmar --}}
        <div>
            <label class="text-[12px] font-semibold text-gray-500 uppercase tracking-wider mb-2 block">Confirmar nueva contrasena</label>
