...
<div class="space-y-5">
    ...
    <div>
        ...
        <p class="text-[13px] text-gray-400 mt-1">Resumen de tu actividad docente y grupos activos.</p>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-gray-100">
            <div class="p-5">
                <div class="flex items-center gap-2 mb-3">
                    ...
                    <span class="text-[13px] font-semibold text-gray-700">Grupos Activos</span>
                </div>
                <p class="text-[22px] font-bold text-gray-900 leading-none">{{ $grupos->count() }}</p>
                <p class="text-[11px] text-gray-400 mt-1.5">{{ $ciclo?->nombre ?? 'Sin ciclo activo' }}</p>
            </div>
            ...
            <div class="p-5">
                <div class="flex items-center gap-2 mb-3">
                    ...
                    <span class="text-[13px] font-semibold text-gray-700">Especialidad</span>
                </div>
                <p class="text-[16px] font-bold text-gray-900 leading-none">{{ $docente?->especialidad ?? 'No asignada' }}</p>
                <p class="text-[11px] text-gray-400 mt-1.5">{{ $docente?->horas_contrato ?? 0 }} hrs/contrato</p>
            </div>
            ...
            <div class="p-5">
                <div class="flex items-center gap-2 mb-3">
                    ...
                    <span class="text-[13px] font-semibold text-gray-700">Evaluacion</span>
                </div>
                <p class="text-[22px] font-bold text-gray-900 leading-none">{{ $docente?->promedio_evaluacion ?? '0' }}</p>
                <p class="text-[11px] text-gray-400 mt-1.5">Promedio de evaluaciones</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm">
            <div class="flex items-center justify-between px-5 pt-5 pb-3">
                <h2 class="text-[15px] font-bold text-gray-800">Grupos del Ciclo</h2>
                <a href="{{ route('docente.grupos') }}" class="text-[11px] font-medium text-gray-400 hover:text-gray-700 px-2.5 py-1 rounded-lg hover:bg-gray-50">Ver todos &rarr;</a>
            </div>
            <div class="px-5 pb-5">
                @if($grupos->isNotEmpty())
                    <div class="space-y-1">
                        ...
                        @foreach($grupos as $grupoId => $horariosDel)
                            ...
                            <div class="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50/70">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-[13px] font-semibold text-gray-800 truncate">{{ $horariosDel->map(fn($h) => $h->materia->nombre_materia)->unique()->join(', ') }}</p>
                                    <p class="text-[11px] text-gray-400">{{ $g->inscripciones_count ?? $g->inscripciones()->count() }} alumnos</p>
                                </div>
                                <a href="{{ route('docente.asistencia.show', $g->id_grupo) }}" class="text-[11px] font-medium text-violet-600 hover:text-violet-800 px-2 py-1 rounded-lg hover:bg-violet-50">Pasar lista</a>
                            </div>
                        @endforeach
                    </div>
                @else
                    <div class="text-center py-8">
                        <p class="text-[13px] text-gray-400">Sin grupos asignados en el ciclo actual.</p>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm">
            <div class="flex items-center justify-between px-5 pt-5 pb-3">
                ...
                <a href="{{ route('docente.noticias') }}" class="text-[11px] font-medium text-gray-400 hover:text-gray-700 px-2.5 py-1 rounded-lg hover:bg-gray-50">Ver todas &rarr;</a>
            </div>
            <div class="px-5 pb-5">
