    ...
    <form method="POST" action="{{ route('docente.horas-culturales.update', $horasCultural->id_registro) }}" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4 dark:bg-gray-800 dark:border-gray-700 dark:shadow-gray-900/20">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="number" name="horas_acumuladas" value="{{ $horasCultural->horas_acumuladas }}" min="0.5" max="100" step="0.5" required class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-[13px] focus:ring-2 focus:ring-sky-300 outline-none dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200">
            </div>
        </div>

        <div>
            <label class="text-[11px] font-semibold text-gray-500 uppercase mb-1 block dark:text-gray-400">Descripcion</label>
            <textarea name="descripcion" rows="3" class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-[13px] focus:ring-2 focus:ring-sky-300 outline-none resize-none dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200">{{ $horasCultural->descripcion }}</textarea>
        </div>
