...
<div class="space-y-5">
    ...
    <div class="flex items-center justify-between">
        <h1 class="text-[22px] font-bold text-gray-900 dark:text-gray-100">Mis Grupos</h1>
        <span class="text-[12px] font-medium text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-gray-700 px-3 py-1.5 rounded-full">{{ $ciclo?->nombre ?? 'Sin ciclo' }}</span>
    </div>
                ...
                <div class="bg-white dark:bg-gray-800 dark:border-gray-700 dark:shadow-gray-900/20 rounded-2xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                    <div class="p-5">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                <p class="text-[14px] font-bold text-gray-800 dark:text-gray-200">{{ $grupo->clave_grupo }}</p>
                                <p class="text-[11px] text-gray-400 dark:text-gray-500">{{ $grupo->carrera?->nombre_carrera ?? 'Sin carrera' }}</p>
                            </div>
                        </div>
                        ...
                        <div class="space-y-2 mb-4">
                            <div class="flex justify-between text-[12px]">
                                ...
                                <span class="font-medium text-gray-700 dark:text-gray-300">{{ $grupo->cuatrimestre }}°</span>
                            </div>
                            <div class="flex justify-between text-[12px]">
                                <span class="text-gray-400 dark:text-gray-500">Alumnos inscritos</span>
                                <span class="font-medium text-gray-700 dark:text-gray-300">{{ $grupo->inscripciones->count() }}</span>
                            </div>
                            <div class="flex justify-between text-[12px]">
                                <span class="text-gray-400 dark:text-gray-500">Materias</span>
                                <span class="font-medium text-gray-700 dark:text-gray-300">{{ $horariosDel->map(fn($h) => $h->materia->nombre_materia)->unique()->join(', ') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="flex gap-2">
                            ...
                            <a href="{{ route('docente.calificaciones.show', $grupo->id_grupo) }}"
                               class="flex-1 text-center text-[11px] font-medium bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-3 py-2 rounded-xl hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">
                                Calificaciones
                            </a>
                        </div>
                    </div>
    ...
    @else
        <div class="bg-white dark:bg-gray-800 dark:border-gray-700 dark:shadow-gray-900/20 rounded-2xl border border-gray-100 shadow-sm p-12 text-center">
            <p class="text-[14px] text-gray-400 dark:text-gray-500">Sin grupos asignados en el ciclo actual.</p>
        </div>
    @endif
