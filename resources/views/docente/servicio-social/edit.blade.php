...
<div class="max-w-2xl space-y-5">
    ...
    <h1 class="text-[22px] font-bold text-gray-900">Editar Servicio Social</h1>

    <form method="POST" action="{{ route('docente.servicio-social.update', $servicioSocial->id_servicio) }}" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
        @csrf @method('PUT')

        <div>
            <label class="text-[11px] font-semibold text-gray-500 uppercase mb-1 block">Alumno</label>
            <p class="text-[14px] font-medium text-gray-800 bg-gray-50 rounded-xl px-3 py-2.5">{{ $servicioSocial->alumno?->nombre_completo }}</p>
        </div>

        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="text-[11px] font-semibold text-gray-500 uppercase mb-1 block">Horas Acumuladas</label>
                <input type="number" name="horas_acumuladas" value="{{ $servicioSocial->horas_acumuladas }}" min="0" step="1" required class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-[13px] focus:ring-2 focus:ring-violet-300 outline-none">
            </div>
            <div>
        ...
        <div>
            ...
            <input type="text" name="institucion" value="{{ $servicioSocial->institucion }}" class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-[13px] focus:ring-2 focus:ring-violet-300 outline-none">
        </div>
