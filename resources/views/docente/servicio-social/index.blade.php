...
<div class="space-y-5">
                    ...
                    @foreach($alumnosConSS as $alumno)
                        ...
                        <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-700/50">
                            <td class="px-5 py-3 text-[13px] font-medium text-gray-800 dark:text-gray-200">
                                ...
                                <span class="block text-[10px] text-gray-400 font-mono">{{ $alumno->matricula }}</span>
                            </td>
                            ...
                            <td class="px-5 py-3 text-center">
                                <div class="flex justify-center gap-1">
                                    ...
                                    <form method="POST" action="{{ route('docente.servicio-social.destroy', $ss->id_servicio) }}" onsubmit="return confirm('¿Eliminar el registro de servicio social de {{ $alumno->nombre_completo }}?')">
                                        @csrf @method('DELETE')
    ...
    @else
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-12 text-center dark:bg-gray-800 dark:border-gray-700 dark:shadow-gray-900/20">
            ...
            @else
                <a href="{{ route('docente.servicio-social.create') }}" class="inline-block mt-4 text-[12px] text-sky-600 hover:text-sky-800 dark:text-blue-400">+ Registrar el primero</a>
            @endif
        </div>
    @endif
