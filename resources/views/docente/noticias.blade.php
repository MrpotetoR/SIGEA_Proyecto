...
<div class="space-y-5">
    ...
    <h1 class="text-[22px] font-bold text-gray-900">Noticias</h1>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:shadow-md transition-shadow">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            <h3 class="text-[15px] font-bold text-gray-800">{{ $noticia->titulo }}</h3>
                            <p class="text-[12px] text-gray-500 mt-1 line-clamp-2">{{ $noticia->contenido }}</p>
                            <p class="text-[11px] text-gray-400 mt-2">{{ $noticia->fecha_publicacion->format('d/m/Y') }} &mdash; {{ $noticia->fecha_publicacion->diffForHumans() }}</p>
                        </div>
                    </div>
                </div>
            @endforeach
    ...
    @else
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-12 text-center">
            <p class="text-[14px] text-gray-400">Sin noticias disponibles.</p>
        </div>
    @endif
