<x-panel title="Dashboard" panelNombre="Panel Director">
    ...
    @else
        <div class="bg-amber-50 border border-amber-200 rounded-2xl p-6 mb-8">
            <p class="text-amber-700 font-medium">No tienes una carrera asignada. Contacta al administrador.</p>
        </div>
    @endif
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-5 mb-8">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            ...
            <p class="text-2xl font-bold text-gray-900">{{ $kpis['total_alumnos'] }}</p>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            ...
            <p class="text-2xl font-bold text-gray-900">{{ $kpis['total_docentes'] }}</p>
            <p class="text-xs text-gray-500 mt-1">Docentes</p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            ...
            <p class="text-2xl font-bold text-green-600">{{ $distribucion_semaforo['verde'] }}</p>
            <p class="text-xs text-gray-500 mt-1">Semaforo verde</p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            ...
            <p class="text-2xl font-bold text-red-500">{{ $distribucion_semaforo['rojo'] }}</p>
            <p class="text-xs text-gray-500 mt-1">Semaforo rojo</p>
        </div>
    </div>

    {{-- Distribucion semaforo --}}
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-8">
        <h2 class="text-[15px] font-semibold text-gray-800 mb-5">Distribucion Semaforo Academico</h2>
        <div class="grid grid-cols-3 gap-6">
            @foreach(['verde' => ['bg-green-500', 'text-green-700', 'bg-green-50'], 'amarillo' => ['bg-yellow-400', 'text-yellow-700', 'bg-yellow-50'], 'rojo' => ['bg-red-500', 'text-red-700', 'bg-red-50']] as $nivel => $colors)
                @php $total = array_sum($distribucion_semaforo) ?: 1; $porcentaje = round(($distribucion_semaforo[$nivel] / $total) * 100); @endphp
                <div class="{{ $colors[2] }} rounded-xl p-4">
    ...
    @if(!empty($indice))
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h2 class="text-[15px] font-semibold text-gray-800 mb-5">Indice de Aprobacion — {{ $ciclo?->nombre }}</h2>
            <div class="grid grid-cols-3 gap-6">
                <div class="text-center p-4 bg-gray-50 rounded-xl">
                    <p class="text-3xl font-bold text-gray-800">{{ $indice['total'] ?? 0 }}</p>
                    <p class="text-xs text-gray-500 mt-1">Total calificaciones</p>
                </div>
                <div class="text-center p-4 bg-green-50 rounded-xl">
                    <p class="text-3xl font-bold text-green-600">{{ $indice['porcentaje_aprobacion'] ?? 0 }}%</p>
                    <p class="text-xs text-gray-500 mt-1">Aprobacion</p>
                </div>
                <div class="text-center p-4 bg-red-50 rounded-xl">
                    <p class="text-3xl font-bold text-red-500">{{ isset($indice['porcentaje_aprobacion']) ? 100 - $indice['porcentaje_aprobacion'] : 0 }}%</p>
                    <p class="text-xs text-gray-500 mt-1">Reprobacion</p>
                </div>
            </div>
        </div>
    @endif
