...
class DocentesController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('gestor.docentes.index', compact('docentes'));
    }

    public function create()
    {
        $carreras = Carrera::misCarreras()->orderBy('nombre_carrera')->get();
        return view('gestor.docentes.create', compact('carreras'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $documentosOpcionales = ['cedula_profesional'];
        foreach (DocumentoDocente::TIPOS as $tipo => $label) {
            $regla = in_array($tipo, $documentosOpcionales, true) ? 'nullable' : 'required';
            $rules["documentos.$tipo"] = "$regla|file|mimes:pdf|max:5120";
        ...
        DB::transaction(function () use ($request) {
            ...
            $docente = Docente::create([
                ...
                'nivel_educativo' => \App\Support\ContextoEducativo::actual() ?? 'universidad',
            ]);
        ...
        return redirect()->route('gestor.docentes.index')->with('success', 'Docente registrado.');
    }

    public function show(Docente $docente)
    {
        $docente->load('user', 'horarios.grupo', 'horarios.materia', 'evaluaciones');
        return view('gestor.docentes.show', compact('docente'));
    }
    ...
    public function edit(Docente $docente)
    {
        ...
        $carreras = Carrera::misCarreras()->orderBy('nombre_carrera')->get();
        return view('gestor.docentes.edit', compact('docente', 'carreras'));
    }
    ...
    public function update(Request $request, Docente $docente)
    {
        ...
        DB::transaction(function () use ($request, $docente) {
            ...
            // Verificación final: todos los documentos obligatorios deben existir (cédula profesional es opcional)
            $documentosOpcionales = ['cedula_profesional'];
            $tiposObligatorios = array_diff(array_keys(DocumentoDocente::TIPOS), $documentosOpcionales);
            $tiposExistentes = DocumentoDocente::where('id_docente', $docente->id_docente)
                ->pluck('tipo')->toArray();
            $faltantes = array_diff($tiposObligatorios, $tiposExistentes);
            if (!empty($faltantes)) {
                $errores = [];
        ...
        return redirect()->route('gestor.docentes.index')->with('success', 'Docente actualizado.');
    }

    public function destroy(Docente $docente)
    {
        $docente->user->update(['activo' => false]);
        return redirect()->route('gestor.docentes.index')->with('success', 'Docente desactivado.');
    }
